...
//                        esc.addSetRelativePrintPositon(rPos);
                esc.addSetRelativePrintPosition(rPos);
                // 设置为倍高倍宽
...
//                        esc.addRastBitImage(bitmap, 576, 0);
                esc.addRasterBitImage(bitmap, 576, 0);
...
//        esc.addUserCommand(bytes);
    ...
    public static Vector<Byte> mapToLabel(Map<String, Object> config, List<Map<String, Object>> list) {
        Log.i(TAG, new JSONObject(config).toString());
        Log.i(TAG, new JSONArray(list).toString());


        LabelCommand tsc = new LabelCommand();


        int page_width = (int) (config.get("width") == null ? 60 : config.get("width")); // 单位：mm
        int page_height = (int) (config.get("height") == null ? 75 : config.get("height")); // 单位：mm
        int gap = (int) (config.get("gap") == null ? 0 : config.get("gap")); // 单位：mm


        // 设置标签尺寸宽高，按照实际尺寸设置 单位mm
        tsc.addSize(page_width, page_height);
        ...
        //
        tsc.addCodePage(LabelCommand.CODEPAGE.PC437);
        // 清除打印缓冲区
        tsc.addCls();
        ...
        for (Map<String, Object> m : list) {
            ...
            if ("text".equals(type)) {
                ...
                tsc.add1DBarcode(x, y, LabelCommand.BARCODETYPE.CODE128, 100, LabelCommand.READABLE.ENABLE, LabelCommand.ROTATION.ROTATION_0, content);
            } else if ("qrcode".equals(type)) {
                ...
                int width = (int) (m.get("width") == null ? 300 : m.get("width"));
                byte[] bytes = Base64.decode(content, Base64.DEFAULT);
                Bitmap bitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.length);
                tsc.addBitmap(x, y, LabelCommand.BITMAP_MODE.OVERWRITE, width, bitmap);
            }
        }
